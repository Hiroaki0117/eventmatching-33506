...
RSpec.describe User, type: :model do
  describe '#create' do
    ...
    context "ユーザー新規登録ができない場合" do
      ...
      it "Eメールが空では入力できない" do
        @user.email = nil
        @user.valid?
        expect(@user.errors.full_messages).to include("Eメールを入力してください")
      end
  
      it "Eメールは@がない場合登録できない" do
        @user.email = "aaaaaa"
        @user.valid?
        expect(@user.errors.full_messages).to include("Eメールは不正な値です")
      end
  
      it "パスワードが空では入力できない" do
        @user.password = nil
        @user.valid?
      ...
      it "パスワードとパスワード（確認用)が不一致では登録ができない" do
        @user.password = "11111a"
      ...
      it "電話番号が半角数字でないと登録できない" do
        @user.phone_number = "１１１１"
        @user.valid?
        expect(@user.errors.full_messages).to include("電話番号は半角数字を入力してください")
      end
